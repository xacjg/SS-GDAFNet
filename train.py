...
from models.door_model import BaseNet
...
def trainValidateSegmentation(args):
    ...
    model = BaseNet(input_nc=3, output_nc=1).to(device)
    ...
    if args.file_root == 'LEVIR':
        args.file_root = '/root/Datasets/LEVIR-CD_256'
    elif args.file_root == 'WHU':
        args.file_root = '/root/Datasets/WHU-CD-256-Organized'
    elif args.file_root == 'SYSU':
        args.file_root = '/root/Datasets/SYSU-CD'
    elif args.file_root == 'CDD':
        args.file_root = '/root/Datasets/ChangeDetectionDataset/Real/subset'
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        print('\n===== 第 %d 轮训练结束 =====' % epoch)
        print('训练集: Kappa = %.4f, IoU = %.4f, F1 = %.4f, R = %.4f, P = %.4f, Loss = %.4f' %
              (score_tr['Kappa'], score_tr['IoU'], score_tr['F1'], score_tr['recall'],
               score_tr['precision'], lossTr))
        ...
        print('验证集: Kappa = %.4f, IoU = %.4f, F1 = %.4f, R = %.4f, P = %.4f, Loss = %.4f' %
              (score_val['Kappa'], score_val['IoU'], score_val['F1'], score_val['recall'],
               score_val['precision'], lossVal))
        ...
        if epoch % 1 == 0 and max_F1_val <= score_val['F1']:
            ...
            print('新的最佳模型! F1: %.4f -> %.4f' % (max_F1_val, score_val['F1']))

        if epoch > 0 and epoch % 10 == 0:
            epoch_model_file = args.savedir + f'model_epoch_{epoch}.pth'
            torch.save(model.state_dict(), epoch_model_file)
            saved_models.append((epoch, epoch_model_file))
            print(f"已保存第{epoch}轮模型到 {epoch_model_file}")
    ...
    print("\n最佳模型测试结果：\t Kappa = %.4f\t IoU = %.4f\t F1 = %.4f\t R = %.4f\t P = %.4f" \
          % (score_test['Kappa'], score_test['IoU'], score_test['F1'], score_test['recall'], score_test['precision']))
    logger.write(
        "\n%s\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f" % ('最佳模型测试', score_test['Kappa'], score_test['IoU'],
                                                          score_test['F1'], score_test['recall'],
                                                          score_test['precision']))
    if saved_models:
        logger.write("\n\n每5轮保存模型的测试结果：")
        logger.write("\n%s\t%s\t%s\t%s\t%s\t%s" % ('轮次', 'Kappa', 'IoU', 'F1', 'R', 'P'))

        for epoch, model_path in saved_models:
            state_dict = torch.load(model_path)
            model.load_state_dict(state_dict)

            loss_test, score_test = val(args, testLoader, model, epoch)

            print(f"\n第{epoch}轮模型测试：\t Kappa = {score_test['Kappa']:.4f}\t IoU = {score_test['IoU']:.4f}\t "
                  f"F1 = {score_test['F1']:.4f}\t R = {score_test['recall']:.4f}\t P = {score_test['precision']:.4f}")

            logger.write("\n%d\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f" %
                         (epoch, score_test['Kappa'], score_test['IoU'], score_test['F1'],
